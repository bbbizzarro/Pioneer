...
public class PlayerController : Character {
	...
	private void GetReferences() {
		...
		interactor = (Interactor)GetNode("Interactor");
		interactor.Initialize(Inventory);
	}
	...
	private void ParseInput() {
		...
		if (Input.IsActionPressed("Select") && interactor.GetCurrent() != null) {
			Vector2 direction = (interactor.GetCurrent().GlobalPosition - GlobalPosition);
			if (direction.Length() > 8f) 
